...
void updateSubmarine() {
 ...
 if ((~((*(volatile unsigned short *)0x04000130)) & ((1<<6))) && submarine.row >= submarine.rdel) {
  submarine.row -= submarine.rdel;
 } else if ((~((*(volatile unsigned short *)0x04000130)) & ((1<<7))) && submarine.row + submarine.height - 1 < 160 - submarine.rdel) {
  submarine.row += submarine.rdel;
 }

 if ((!(~(oldButtons)&((1<<1))) && (~buttons & ((1<<1)))) && submarine.bulletTimer >= 20) {
   fireBullet();
...
void drawBullet(BULLET* b) {
 if(b->active) {
  drawRect(b->oldCol, b->oldRow, b->width, b->height, ((12) | (20)<<5 | (29)<<10));
...
void updateFishy() {
 if (fishy.active) {
  ...
  if (fishy.row <= 0 || fishy.row + fishy.height-1 >= 140)
   fishy.rdel *= -1;
  if (fishy.col <= 50 || fishy.col + fishy.width >= 240 - 5)
   fishy.cdel *= -1;
